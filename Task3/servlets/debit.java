	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			pstmt1 = con.prepareStatement(
					"update accounts set balance=balance-? where password=? and accno=? and balance>=?;");
			...
			if (i > 0 && rs.next()) {
				...
				pstmt2 = con.prepareStatement(
						"insert into transaction (name,accno,type,status,amount,balance,date_time) values (?,?,'withdraw',?,?,?,?);");
